...
class IsarCategoryRepository implements CategoryRepository {
  ...
  IsarCategoryRepository(this.db);
    ...
    Future<List<Category>> getAllCategories() async {
      ...
      final categories = await db.isarCategories.where().findAll();
  
      // Convert IsarCategory to Category using the toDomain method
    ...
    Future<Category?> getCategoryById(String id) async {
      ...
      final category = await db.isarCategories.get(int.parse(id));
    ...
    @override
    Future<Category?> getCategoryByIdForCurrentMonth(String id, String month, String year) async {
      // Fetch a category by its ID for the current month and year from the Isar database asynchronously.
      final category = await db.isarCategories
          .filter()
    ...
    @override
    Future<List<Category?>> getCategoriesForCurrentMonth(String month, String year) async {
      // Fetch categories for the current month and year from the Isar database asynchronously.
      final categories = await db.isarCategories
          .filter()
          .monthEqualTo(month)
          .yearEqualTo(year)
          .findAll();
    ...
    Future<void> addCategory(Category category) async {
      // Convert Category to IsarCategory using the fromDomain method
      // and add it to the Isar database asynchronously.
      await db.writeTxn(() async {
        await db.isarCategories.put(IsarCategory.fromDomain(category));
      });
    }
    ...
    Future<void> deleteCategory(String id) async {
      ...
      await db.writeTxn(() async {
        await db.isarCategories.delete(int.parse(id));
      });
    }
}
