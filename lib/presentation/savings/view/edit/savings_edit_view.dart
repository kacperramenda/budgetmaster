...
class SavingEditView extends StatefulWidget {
  ...
  const SavingEditView({super.key, required this.saving});
...
class _SavingEditViewState extends State<SavingEditView> {
  ...
  void initState() {
    super.initState();
    _nameController = TextEditingController(text: widget.saving.name);
    _amountController = TextEditingController(text: widget.saving.amount.toString());
    _descriptionController = TextEditingController(text: widget.saving.description ?? '');
    
    final date = widget.saving.date;
    _dateController = TextEditingController(
      text: '${date.day.toString().padLeft(2, '0')}/${date.month.toString().padLeft(2, '0')}/${date.year}'
    );
    
    savingDate = _dateController.text;
    savingSafeId = widget.saving.safeId;
    savingAmount = _amountController.text;
    savingDescription = _descriptionController.text;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          const PageHeader(title: "Edytuj oszczędność"),
          Padding(
              ...
              children: [
                            ...
                            children: [
                              ...
                              ...safes.skip(1).map((safe) {
                                final isSelected = selectedSafe?.id == safe.id || 
                                    (selectedSafe == null && safe.id == widget.saving.safeId);
                                return Padding(
                                  padding: const EdgeInsets.only(right: 8),
                                  child: Container(
                ...
                ButtonPrimary(
                  ...
                  onPressed: () async {
                    ...
                    final updatedSaving = widget.saving.copyWith(
                      name: name,
                    ...
                    await context.read<SavingsCubit>().updateSaving(updatedSaving, context);
